  ...
  for (const item of items) {
    try {
      ...
      if (item.encrypted_value && item.iv_value_hex) {
        ...
        patch.encrypted_value = nv.ciphertext
        patch.iv_value_hex    = nv.ivHex
      }
      await supabase.from('items').update(patch).eq('id', item.id)
      count++
    } catch { /* item corrotto, salta */ }
  }
  return count
}
...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      register: async (email, password) => {
        ...
        try {
          if (isDisposableEmail(email))
            throw new Error('Email usa-e-getta non consentita.')
          ...
          const { data, error } = await supabase.auth.signUp({
            ...
            password,
            options: {
              // URL corretto con il base path di GitHub Pages
          ...
          if (data.session) {
            ...
            set({ loading: false,
              error: '📧 Controlla la tua email e clicca il link di conferma, poi accedi.' })
          }
        } catch (e) {
      ...
      login: async (email, password) => {
        ...
        try {
          ...
          if (cfg.iv_base_hex?.trim()) {
            ...
            if (!ok) throw new Error('Password errata o account cifrato con credenziali diverse.')
          }
      ...
      // ── CAMBIO PASSWORD CON RI-CIFRATURA ───────────────────────────────────
      changeCryptoPassword: async (email, oldPwd, newPwd) => {
        ...
        try {
          ...
          const { error: rpcErr } = await supabase.rpc('update_my_crypto_config', {
      ...
      requestAccountDeletion: async (email) => {
        ...
        try {
          if (isDisposableEmail(email)) throw new Error('Email non valida.')
          const { error } = await supabase.auth.resetPasswordForEmail(email, {
            redirectTo: appUrl('/delete-account'),
          })
          if (error) throw new Error(error.message)
          set({ loading: false })
        } catch (e) {
      ...
      initSession: async () => {
        ...
        if (data.session?.user) {
          set({ user: data.session.user, isAuthenticated: true,
            cryptoReady: false, isAdmin: isAdmin(data.session.user.email) })
        }
      },
    }),
    {
